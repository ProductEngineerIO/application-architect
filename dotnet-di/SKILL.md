---
name: di
description: Evaluate, review, and validate .NET Dependency Injection implementations. Use when assessing DI patterns, reviewing service registration, evaluating constructor injection, or validating architecture stories involving dependency management.
---

# Dependency Injection Guidance

You are evaluating or helping with Dependency Injection patterns in .NET. This skill has four contexts: Evaluation, Review, Writing/Learning, and Validation.
...
> What would you like to do with Dependency Injection?
> A) Evaluate my existing DI implementation
> B) Review a specific code change
> C) Learn/write DI patterns
> D) Validate a story or requirement

Only skip this and go straight to a context if the user's triggering message already made it obvious (e.g. "review my DI changes" clearly means Review; a bare `/dotnet-di` does not).

## Evaluation Context

Use when the user asks to **evaluate** their DI implementation.
...
1. If you have access to the codebase (files are available to read/search), just start looking: service registrations (`Startup.cs`, `Program.cs`, `*Extensions.cs`), constructors across services, the DI container setup, and lifetime declarations (`AddSingleton`, `AddScoped`, `AddTransient`).
2. Only ask the user a clarifying question first if you have no code access at all, or if the codebase is large enough that you need direction (e.g. "Which project/service should I focus on?").
3. Once you've looked, report **specific findings**, not a generic score:
   - Name actual services/classes and what you found (e.g. "`OrderService` takes a `new HttpClient()` directly instead of being injected")
   - Flag concrete risks: lifetime mismatches (e.g. captive dependencies — a singleton holding a scoped service), missing registrations, service locator anti-pattern usage, constructors doing more than accepting dependencies
   - Give a score (0-100) as a summary *after* the findings, grounded in what you actually saw — not derived from a self-report
...
- or name another topic directly (e.g. "let's look at dependency inversion instead")
...
Use when the user asks to **review** DI code or changes.
...
Give specific, line/file-referenced feedback tied to what's actually in the diff or pasted code — not generic DI advice.

## Writing/Learning Context

Use when the user asks to **learn** or **write** DI patterns.
...
Use when the user asks to **validate** a story or requirement involving DI.
